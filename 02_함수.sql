...
-- NVL(컬럼명, 컬럼값이 NULL 일떄 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경

/* NULL과 산술 연산을 진행하면 결과는 무조건 NULL */
SELECT EMP_NAME, SALARY, BONUS, NVL(BONUS, 0), SALARY + (SALARY * NVL(BONUS, 0))
FROM EMPLOYEE;
...
-- EMPLOYEE테이블에서 보너스를 받으면 'O', 안받으면 'X' 저허;
...
DECODE(JOB_CODE, 'J7', SALARY * 1.2, 'J6', SALARY * 1.15, 'J5', SALARY * 1.1 , SALARY * 1.05)
"인상된 급여"
FROM EMPLOYEE;
