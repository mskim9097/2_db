...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); == 내부, 등가 조인
...
AND LOCATION_ID = LOCAL_CODE AND JOB_NAME = '대리' 
...
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
JOIN JOB USING (JOB_CODE)
WHERE EMP_NO LIKE '7%' AND SUBSTR(EMP_NO, 8, 1) = '2'
AND EMP_NAME LIKE '전%';
...
SELECT EMP_NAME, JOB_NAME, SALARY "급여", SALARY * NVL2(BONUS, 1 + BONUS, 1) * 12 "연봉(보너스포함)"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
FROM EMPLOYEE E1
LEFT JOIN EMPLOYEE E2 USING (DEPT_CODE)
